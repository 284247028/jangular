...
function angularizeAttrs(e, scope) {
    ...
    for (var attr in attrs) {
        ...
        // values to determine if we should strip out the attribute
        var isUib = attr.substring(0, 4) === 'uib-';
        var attrPrefix = attr.substring(0, 3);
        var attrShortPrefix = attr.substring(0, 2);
        var isGhDirective = attrShortPrefix === 'f-' || attrShortPrefix === 'b-' || attrPrefix === 'bf-';
        var isAngularAttr = attrPrefix === 'ng-';
        var isGhAttr = attrPrefix === 'gh-';
        var isTransform = transforms.hasOwnProperty(attr);
        var isIgnore = transformsIgnored.hasOwnProperty(attr);
        var isStandardStrip = isAngularAttr || isTransform || isIgnore;
        var isAmpStrip = isStandardStrip || isGhAttr || isGhDirective;
        ...
        if (!(transforms[attrNormal] || transformsIgnored[attrNormal])) {
            ...
            if (isUib || (scope.isAmp && isAmpStrip)) {
                delete e.attributes[attr];
            }
        ...
        // if amp and amp strip OR is scope strip and standard strip then remove attribute
        if ((scope.isAmp && isAmpStrip) || (scope[JNG_STRIP] === true && isStandardStrip)) {
            delete e.attributes[attr];
        }
    }
}
