...
var jngFnStartRegex = /\"__jng_fn_start__/g;
var jngFnEndRegex = /__jng_fn_end__\"/g;

function stringifyIncludeFunctions(key, value) {
    if ( typeof value === 'function' ) {
        return jngFnStart + value.toString() + jngFnEnd;
    }
    return value;
}
...
function angularize(e, currentModel) { // e is an already-built Jeff element w attributes, children, and text - and a tagName...
    ...
    for ( var prop in currentModel ) {
        var stringified = JSON.stringify(currentModel[prop], stringifyIncludeFunctions);
        // remove our function delimiters and also newline characters, which in JS sometimes imply the end of a statement
        stringified = stringified.replace(jngFnStartRegex, '').replace(jngFnEndRegex, '').replace(/\\n/g, '').replace(/\n/g,'');
        buildVars.push(prop + ' = ' + stringified);
    }
    if ( buildVars.length > 0 ) {
        foreval = 'var ' + buildVars.join(', ') + ';';
    }
    //console.log('eval: ' + foreval);
    eval(foreval);
