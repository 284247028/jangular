...
var Jeff = require('jeff-core'),
    ...
    JNG_UI_VIEW_RENDERING = '__jng_ui_view_rendering';
...
var angularTransforms = {
    ...
    'count': function(e, val) {
        if (e.attributes && e.attributes.when && e.attributes.hasOwnProperty('ng-pluralize')) {
            var when = null;
    ...
    /*
    'ui-view': function(e, val, model) {
        model[JNG_UI_VIEW_RENDERING] = true;
        return e;
    }
    */
};
...
function deangularizeNode(e) {

    var attr = null, i, len, attrs = e.attributes;

    if (Jeff.Utils.isArray(e)) {
        for (i = 0; i < e.length; i++) {
            deangularizeNode(e[i]);
        }
        return e;
    }

    if (attrs) { // find each angular directive and transform
        for( attr in attrs ) {
            var attrNormal = attr.replace(patternPrefix,'').replace(patternDelimit, '-');
            if ( attrNormal.indexOf('ng-') === 0 || angularTransforms.hasOwnProperty(attrNormal) ) {
                delete(e.attributes[attr]);
                continue;
            }
        }
    ...
    if (e.children) {
        for( i = 0, len = e.children.length; i < len; i++ ) {
            e.children[i] = deangularizeNode(e.children[i]);
        }
    }
    return e;
    ...
    function angularizeNode(e) { // we don't need to spill model each time, hence inner function
        ...
        if (attrs && !e.ngRepeat && (attrs['ng-repeat'] || attrs['data-ng-repeat'] || attrs['jng-repeat'] || attrs['data-jng-repeat'])) {
            ...
            for( attr in attrs ) {
                ...
                if ( attr === 'ui-view' ) {
                    needsStripping = true;
                } else if ( angularTransforms[attrNormal] ) {
                    if ( currentModel[JNG_UI_VIEW_RENDERING] ) {
                        delete(e.attributes[attr]);
                        continue;
                    }
                    var evaled = null;
        ...
        if (e.ngRepeat) { // ng-repeat gets pretty special treatment and re-scoping (see use of outer function)...
            ...
            var transformNode = (needsStripping ? deangularizeNode : angularizeNode);
            for( i = 0, len = e.children.length; i < len; i++ ) {
                e.children[i] = transformNode(e.children[i]);
            }
        }
        return e;
    }
...
ngRepeat = function(e, model) {
    ///^\s*([\s\S]+?)\s+in\s+([\s\S]+?)(?:\s+track\s+by\s+([\s\S]+?))?\s*$/   - got that?
    ...
    if (repeatArr) {
        for ( var i = 0, len = repeatArr.length; i < len; i++ ) {
            ...
            if ( i > 0 ) {
                if (e.attributes['jng-repeat']) {
                    e.attributes['jng-repeated'] = e.attributes['jng-repeat'];
                    delete(e.attributes['jng-repeat']);
                } else if (e.attributes['data-jng-repeat']) {
                    e.attributes['data-jng-repeated'] = e.attributes['data-jng-repeat'];
                    delete(e.attributes['data-jng-repeat']);
                }
            }
