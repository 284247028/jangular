...
var angularTransforms = {
    ...
    'count': function(scope, element, attrs, val) {
        if (attrs && attrs.hasOwnProperty('when') && attrs.hasOwnProperty('ng-pluralize')) {
            var when = null;
            ...
            if ( when ) {
                ...
                var result = (when[val] ? when[val] : (val === 1 && when['one'] ? when['one'] : (when['other'] ? when['other'] : '')));
                result = result.replace(patternPluralize, counted);
                element.empty();
                element.text(result);
            }
        }
