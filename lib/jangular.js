...
var angularTransforms = {
    ...
    'ng-pluralize': function (e, val, model) {

        // if no count or when, then return e without doing anything
        if (!e.attributes || !e.attributes.hasOwnProperty('count') || !e.attributes.hasOwnProperty('when')) {
            return e;
        }
        ...
        var when = null, counted = 0;
        eval('when = ' + e.attributes.when + ';');
        eval('counted = model.' + e.attributes.count + ';');
