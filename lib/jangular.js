...
function getDirectiveAttrs(e, directiveName, val) {
    ...
    attrs.$observe = function(attrName, fn) {
        if ( attrName === directiveNameCamel || attrName === directiveName ) {
            fn(val); // $observe should give back the evaluated version
        } else {
            fn(attrs[attrName]);
        }
    };
...
Jeff.templateToString = function(result, model, strip) {
    model = model || {};
    if (strip) { model[Jeff.JNG_STRIP_DIRECTIVES] = true; }
    var res = (Jeff.Utils.isString(result) ? result : (Jeff.Utils.isArray(result) ? angularize(Jeff.naked(result), model).toString() : angularize(result, model).toString()));
    if (strip) { delete model[Jeff.JNG_STRIP_DIRECTIVES]; }
    return res;
};
...
var makeDirective = {
    'replace': function (name, opts) {
        ...
        return function(scope, element, attrs) {
            ...
            if (name === 'gh-questionlist') {
                console.log('got here');
            }

            var rendered = directive(newModel, element, attrs);

            //TODO: refactor these because right now gh and pancakes put EVERYTHING through replace directive
            if (rendered) {
...
Jeff.addDirectives = function (directives) {
    for (var directiveName in directives) {
        if (directives.hasOwnProperty(directiveName) && !angularTransforms[directiveName]) {
            var directive = directives[directiveName];

            if ( Jeff.Utils.isFunction(directive) ) {
                angularTransforms[directiveName] = directive;
            }
            else {
